...
results_dir <- here("NBAtlas", "01_dataset_eda", "bar_plots")
...
for (i in colnames(metadata)) {
  ...
  adjusted_max_y <- max_y * 1.2
  
  # Create and print the plot
  print(
    ggplot(metadata_summary, aes(x = as.factor(metadata_value), y = n, fill = as.factor(metadata_value))) +
